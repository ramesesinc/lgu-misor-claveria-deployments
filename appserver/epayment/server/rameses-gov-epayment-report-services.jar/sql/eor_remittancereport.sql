	...
	select 
		...
		(case 
			when t1.payable = 1 then 
				(select acctname from vw_remittance_eor_share where receiptid = t1.receiptid and acctid = t1.acctid limit 1)
			else 
				(select acctname from vw_remittance_eor_item where receiptid = t1.receiptid and acctid = t1.acctid limit 1)
		end) as particulars
	from ( 
		select 
	...
	select 
		...
		(case 
			when t0.payable = 1 then 
				(select acctcode from vw_remittance_eor_share where receiptid = t0.receiptid and acctid = t0.acctid limit 1)
			else 
				(select acctcode from vw_remittance_eor_item where receiptid = t0.receiptid and acctid = t0.acctid limit 1)
		end) as acctcode, 
		(case 
			when t0.payable = 1 then 
				(select acctname from vw_remittance_eor_share where receiptid = t0.receiptid and acctid = t0.acctid limit 1)
			else 
				(select acctname from vw_remittance_eor_item where receiptid = t0.receiptid and acctid = t0.acctid limit 1)
		end) as acctname
	from ( 
		select receiptid, fundid, acctid, sum(amount) as amount, 0 as payable
		from vw_remittance_eor_item 
